    ...
    if (isAuthUser) {
      ...
      if (isCurrentCartItemAlreadyExists?.length > 0) {
        return NextResponse.json({
          success: false,
          message:
            "Container is already added in cart! Please add different Container",
        });
      }
      ...
      if (saveProductToCart) {
        return NextResponse.json({
          success: true,
          message: "Container is added to cart !",
        });
      } else {
        return NextResponse.json({
          success: false,
          message: "failed to add the container to cart ! Please try again.",
        });
      }
    } else {
      return NextResponse.json({
